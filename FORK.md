...
| | |
|---|---|
| Upstream commit | **`main` @ `3528a428`** (2026-09-23, "Add support for conditional Virtual MCP Targets (#3615)"; 260 commits past v1.5.0). A `main` commit, not a release tag: upstream has released nothing stable since v1.5.0 (2026-08-27; `v1.6.0-alpha.1` of 2026-09-14 predates what the Substrate line needs, `v1.6.0-alpha.2` of 2026-09-22 is a pre-release), and the Substrate line's pin needs what `main` carries (below). The line returns to a release tag at the first upstream stable release that contains `8dba3989` (v1.6.0 by upstream's cadence). |
| Why this one | The Substrate line (`giantswarm/substrate`) is pinned to kagent-dev/substrate **v0.2.0-beta4**, whose chart runs `atenet-router` and `atenet-egress` on `ghcr.io/agentgateway/agentgateway:v0.0.0-alpha.8dba3989` — a nightly of upstream commit `8dba3989` ("substrate: align credential injection with provider contract (#3524)"), the egress credential injection kagent's runtimes rely on since kagent-dev/kagent#2868. Any `main` commit ≥ `8dba3989` serves; the line takes `main`'s head of the day the three lines move together (agentgateway → Substrate → kagent, giantswarm/giantswarm#37742). The 78 commits between `8dba3989` and the pin touch nothing under `crates/agentgateway/src/http/substrate/`. kagent-dev/substrate v0.2.0-beta5, the Substrate line's pin since 2026-09-24, runs the same `8dba3989` nightly. The packaging chart `giantswarm/agentgateway` follows the same release for the platform's controller and data plane (see "Consumers"). |
| When it moves | with the consumers, proven in agentlab first — see "Re-pin". Not on a schedule. Previous pins: v1.5.0 (2026-09-10 → 2026-09-14, releases `v1.5.1-gs.1`–`gs.3`), `main` @ `c1d24607` (2026-09-14 → 2026-09-22, releases `v1.5.1-gs.4`, `v1.5.1-gs.5`, `2.0.0`), `main` @ `08f4563b` (2026-09-22 → 2026-09-24, releases `2.1.0`, `2.1.1`, `2.1.2`); this pin's release is `2.2.0` (2026-09-24). |
...
therefore moved this line onto `main` ≥ `8dba3989` first; the Substrate chart pins this line's `2.1.0`, then `2.1.2`. The Substrate line's re-pin onto v0.2.0-beta5 (2026-09-24) needs nothing new from this line (beta5 runs the same `8dba3989` nightly) and keeps the chart at `2.1.2`.
...
|---|---|---|---|
| Admit a `RESUMING` actor at CONNECT time next to `RUNNING` (SUSPENDED, PAUSED, CRASHED and DELETING stay refused; the denial names the state) | Substrate commits `RUNNING` only after the workload served readyz; a workload that fetches what it needs to become ready — kagent's Go ADK and Claude harnesses materialise git skills before readyz — was refused (`403 Forbidden: actor is not running`) and its ActorTemplate never got its golden snapshot ([#37742](https://github.com/giantswarm/giantswarm/issues/37742) row 8; acceptance test `agentlab skills-test`, [agentlab#137](https://github.com/giantswarm/agentlab/issues/137)). Counterpart of the Substrate line's [giantswarm/substrate#4](https://github.com/giantswarm/substrate/pull/4) (ateom arms the tunnel before the first container starts, atenet's `ext_proc` admits `RESUMING`) | `37b1ac49` (the `substrate: admit a resuming actor's egress at CONNECT time` commit, replayed clean at the 2026-09-22 and 2026-09-24 re-pins; on the v1.5.0 base it was the same change in `egress.rs`, pull request #4 — at the 2026-09-14 re-pin the prepared main-shaped commit `7771e400` of `upstream/substrate-egress-resuming` replaced it, the check having moved to `egress_actor_resolution.rs` in #3318) | to file: the upstream-shaped patch is branch [`upstream/substrate-egress-resuming`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/substrate-egress-resuming) here (`7771e400`, on the mirror `main` @ `fddff503`; the carried commit is this commit replayed); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it (row 8) |
| Translate a `GRPCRoute` service-only method match to a path prefix and honour `type: RegularExpression` (`CreateAgwGRPCPathMatch` in the controller's translator): a service-only match became `PathMatch_Exact "/<service>/"`, a path no `/Service/Method` request has, so the rule never matched; a `RegularExpression` match with a service was emitted as an exact match on the regex text | the platform routes a gRPC API through a `GRPCRoute` that matches its services (so a new RPC needs no route change) — with the defect every call fell through to the catch-all `HTTPRoute` behind it (found on agentgateway v1.5.0 controller and proxy, identical on `main` @ the pin); until the fix is on the line the chart enumerates every service+method pair as exact matches | `223b80e2` (the `fix(controller): translate GRPCRoute service-only and RegularExpression method matches` commit of pull request #6, replayed clean at the 2026-09-14, 2026-09-22 and 2026-09-24 re-pins) | to file: no upstream issue or pull request covers it (searched 2026-09-11); the upstream-shaped patch is branch [`upstream/grpcroute-method-match-translation`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/grpcroute-method-match-translation) here (on the mirror `main`); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it (row 25). Falls away at the re-pin onto the first release containing it |
| Name the limit and the declared size when an MCP upstream's response exceeds the frontend's buffer (`ClientError::ResponseTooLarge` in `crates/agentgateway/src/mcp/mod.rs`, raised by the streamable HTTP upstream's JSON read and the legacy SSE forward's): `upstream response body of N bytes exceeds the maximum buffer size of L bytes` instead of `body exceeded buffer limit` / `length limit exceeded` | the platform routes muster's MCP endpoint through the data plane, which buffers every tool answer whole to `frontend.http.maxBufferSize`; a whole-registry dry run of the platform manager above the 2 MiB default was lost with an error naming neither number, and nothing told the operator which knob bounds it ([giantswarm/agent-platform#630](https://github.com/giantswarm/agent-platform/issues/630); the connectivity chart now renders the platform's own size). Upstream gave the request side its 413 with the limit in agentgateway#3593 (in the pin); this is the response side | `3afc3c82` (the squash of pull request #17: `mcp: name the limit and the size when an upstream response exceeds the buffer (#17)`, replayed clean at the 2026-09-24 re-pin; in release 2.1.0) | to file: no upstream issue or pull request covers the response side (searched 2026-09-23: #3578/#3593 are the request side, #1410/#1413 the limit's propagation); the upstream-shaped patch is branch [`upstream/mcp-response-buffer-error-v2`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/mcp-response-buffer-error-v2) here (one commit — the change and its test — on the mirror `main` @ the pin; the first `upstream/mcp-response-buffer-error` predates the test fix); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it. Falls away at the re-pin onto the first release containing it |
| Read a file resource from disk on every managed fetch (`ResourceManager::fetch_and_wait_normalized` in `crates/agentgateway/src/resource_manager.rs`): a config reload, triggered by any one resource's change, rebuilds every file-backed TLS config from the files' current content instead of the resource cache | Substrate's egress and router gateways take their client certificate for the API server from a Kubernetes projected `podCertificate` volume the kubelet rotates every 24 h by swapping the `..data` symlink; on an installation the servicedns bundle's watch reloaded the config before kubelet had written the rotated podidentity bundle, and nothing read that bundle again (it was no managed resource: the next row), so every sandbox CONNECT was refused with `CertificateExpired` until a restart ([giantswarm/giantswarm#37915](https://github.com/giantswarm/giantswarm/issues/37915), sub-issue of #37757). Reading a local file on a reload is cheap; HTTP resources keep the cache. Reaches managed files only: the Substrate policies' own TLS files became managed with #23 | `7932909f` (the squash of pull request #20: `resource_manager: read a file resource from disk on every managed fetch (#20)`, replayed clean at the 2026-09-24 re-pin; in release 2.1.1) | to file: the upstream-shaped commit and its rustfmt follow-up are branch [`upstream/backend-tls-reload`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/backend-tls-reload) here (on the mirror `main` @ the pin); no upstream issue or pull request covers it (searched 2026-09-23); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it. Falls away at the re-pin onto the first release containing it |
| Resolve the e2e warm step's `-toolexec` helper by absolute path (`controller/test/setup/setup-kind-ci.sh`, through the script's `REPO_ROOT`): the go command runs asm actions from the package's directory and never absolutizes `-toolexec`, so on a cold build cache the first dependency with assembly files (`github.com/modern-go/reflect2`) failed with `fork/exec ./tools/go-compile-without-link: no such file or directory` and the `controller-e2e` lane was red | the suite runs on GitHub-hosted runners whose Actions caches sit at GitHub's 10 GB limit; once the `go-build-controller-e2e` cache was evicted, every pull request's e2e lane failed in the warm step while the consumed branch, whose runs still had the cache, stayed green (pull request #19, four runs). A warm cache hides the defect upstream: no asm action runs | `a2a16cea` (the squash of pull request #21, a single commit: `ci: resolve the warm-test toolexec helper by absolute path (#21)`, replayed clean at the 2026-09-24 re-pin; CI only, in no release) | to file: the upstream-shaped commit with DCO sign-off is branch [`upstream/ci-toolexec-absolute`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/ci-toolexec-absolute) here (`f7edd4a4` on the mirror `main` @ the pin); no upstream issue or pull request covers it (searched 2026-09-23); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it (row 55). Falls away at the re-pin onto the first upstream commit containing it |
| Make the files a config parse reads managed dependencies (`record_files_only_reads` around the deserialization in `NormalizedLocalConfig::from`, `ResourceFetcher::register_file_dependencies` inside the load's scope) and keep a managed file's periodic re-read armed after a watch event (`refresh_file`) | a backend TLS file referenced only under a Substrate policy's `policies.backendTLS` is resolved by upstream's serde hook `de_from_local_backend_policy` with a files-only fetcher, outside the resource manager: no watch, no re-read, not active, re-read only when some other resource triggers a reload. kubelet rotates a gateway pod's two `podCertificate` bundles in separate syncs up to 90 s apart; the listener bundle's watch reloaded before the client bundle was written, and the client certificate expired in place ([giantswarm/giantswarm#37915](https://github.com/giantswarm/giantswarm/issues/37915)). #20 did not reach these files, and its periodic re-read stopped after a file's first inotify event. Lab reproduction on 2.1.1 with 1 h certificates: the egress pod's two bundles land in one kubelet sync and its reload follows both; the router's servicedns rotation runs about 90 s ahead of its podidentity one, its reload re-parses the config before the old client certificate expires, and it held three rotations for that spacing alone, which a 24 h lifetime does not promise . On 2.1.2 (Substrate 1.0.3 chart, 3600 s) kubelet wrote the four bundles in four separate syncs, 69 s to 3.5 min apart, and every write was followed by its own `resource changed, reloading` for that file about 300 ms later, the podidentity files included; the functional confirmation: the lab's `agents-test` passed 6/6 at 22:39Z after the previous certificates had expired at 22:35Z, with no `CertificateExpired` in the egress log and no egress-tunnel failure in the worker logs, and the router reloaded again on its next podidentity cycle (Substrate release-1.0 v1.0.3 pins 2.1.2; agent-platform#653 carries floor 1.0.3 and the platform's own 2.1.2 tags) | `6e875b1e` (the commit of pull request #23, rebase-merged onto `giantswarm` as one commit; at the 2026-09-24 re-pin its parse hook was re-seated on upstream's new YAML parser, #3639; in release 2.1.2) | to file together with #20's: the upstream-shaped commit with DCO sign-off is branch [`upstream/parse-time-file-dependencies`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/parse-time-file-dependencies) here (`b2f5ca0b`, stacked on `upstream/backend-tls-reload` on the mirror `main` @ the pin); no upstream issue or pull request covers it (searched 2026-09-23); a team member opens the agentgateway pull request with DCO sign-off once #37742 has reviewed it (row 53). Falls away at the re-pin onto the first upstream commit containing it |
| Pin `google.golang.org/grpc` at v1.83.2 in `go.mod` (`build: pin google.golang.org/grpc to v1.83.2 (GO-2026-6443)`) | upstream #3614 (in the pin) raised grpc to v1.84.0, which carries [GO-2026-6443](https://pkg.go.dev/vuln/GO-2026-6443) (a server panic on a request without `:authority` and `Host`, GHSA-2v4p-qf9q-27wj): the fix exists only on grpc's `master` (`v1.85.0-dev`), while v1.83.2 is fixed. The line's `govulncheck` lane is a required check of `ci-ok`, and the controller's xDS server is the flagged call path | `9c5fa93d` (the 2026-09-24 re-pin's own commit) | not for upstream as such: falls away at the re-pin onto the first upstream commit that requires a grpc release with the fix (upstream bumps grpc through its dependency automation; a grpc release of the 1.84 line or v1.85.0 closes it) |
| Answer a resume refusal the Substrate control plane marks not resumable at once, with the control plane's own status (`http/substrate/ingress.rs`, `proxy/mod.rs`) | Substrate refuses to resume an actor whose ActorTemplate lost its golden snapshot (an actor created before its WorkerPool moved to a new worker image) with `FailedPrecondition`, the code it also answers for an actor in transition, which the ingress retries while it parks a request: the caller waited out the parking budget (30 s) and got `UNAVAILABLE` "actor … request timed out", the gRPC code derived from an HTTP 504 ([giantswarm/substrate#79](https://github.com/giantswarm/substrate/issues/79)). The control plane marks such a refusal with the ErrorInfo directive `resumable: "false"` in its `substrate.dev` domain ([giantswarm/substrate#80](https://github.com/giantswarm/substrate/pull/80)); the ingress answers it at once as `ProxyError::SubstrateResumeRefused`: a gRPC caller gets the control plane's code, message and `grpc-status-details-bin`, an HTTP caller the code's HTTP status (400). A refusal without the directive is parked and retried as before | [#29](https://github.com/giantswarm/agentgateway-upstream/pull/29) | to file: the upstream-shaped commit is branch [`upstream/substrate-resume-not-resumable`](https://github.com/giantswarm/agentgateway-upstream/tree/upstream/substrate-resume-not-resumable) here, queued in giantswarm/giantswarm#37742 together with Substrate's directive (substrate branch `upstream/golden-snapshot-unavailable`) |
...
Five patches change agentgateway's behaviour beyond upstream `main` at the pin: the admission of a resuming actor,
the `GRPCRoute` method-match translation, the named buffer error on an MCP upstream's response, the file resources
read from disk on every reload and the parse-time files made managed dependencies, all written for upstream and leaving at the first upstream commit that carries them. The sixth carried commit, the e2e warm step's absolute `-toolexec` path, changes no behaviour
and leaves the same way; the seventh, the grpc pin at v1.83.2, changes a dependency version only and leaves when upstream
requires a fixed grpc release. Nothing was dropped at the 2026-09-24 re-pin: upstream merged none of the carried patches
between `08f4563b` and `3528a428`. Dropped at the 2026-09-22 re-pin because the pin contains them: the `cherry-pick -x` of upstream
...
The 2026-09-24 re-pin (`main` @ `08f4563b` → `main` @ `3528a428`) by hand as well, in a scratch worktree: `git rebase
--onto upstream/main 08f4563b` replayed all 27 carried commits and conflicted once, in the parse-time file dependencies
patch (#23) — upstream #3639 replaced the YAML library (`serdes::yamlviajson::from_str` → `serdes::yaml::from_str`) on the
line the patch wraps in `record_files_only_reads`; resolved to the patch on the new parser. `cargo fmt --check`, `go build
./...` and the controller's Go tests locally; the suite in CI on the `sync/20260924-3528a428` push, where `govulncheck`
flagged upstream's grpc v1.84.0 (GO-2026-6443) and the line gained its grpc pin (above); the force-push by a repository admin — who first lifts the admin bypass of the
...
|---|---|---|
| **2.2.1** (2026-09-25, tag `v2.2.1` on `13325b82` = 2.2.0 + a resume refusal the Substrate control plane marks not resumable answered at once with its own status (#29, rebase); a patch of the line for a carried fix; CircleCI `publish` workflow of the tag green, `devctl release wait` exit 0; proven in agentlab as atenet-router and atenet-egress in the giantswarm/substrate#79 upgrade proof) | `main` @ `3528a428` | `agentgateway-upstream/agentgateway:2.2.1` `sha256:b3decd06d41ae4b9124210bfee2fb8529d704333827dfd8402c5e7eb1544d65c` (linux/amd64 `sha256:9a33e808e1813bb06d20db47121af985b0d600cd2f0357573c27827e326a758e`, linux/arm64 `sha256:3096db7f77406f8285534a1b2a4fb8abea90f6eba9ef837c7050a4874374ec92`) · `agentgateway-upstream/controller:2.2.1` `sha256:cfa5fedd5e3f6e1ca36507ee354238e63de368e7b0bdadb8ea146aeb17cc2157` (linux/amd64 `sha256:c7e9f07b216d55432b5c45adfa10925821a739a89dfaa5f5dc7b282a20a4f5ce`, linux/arm64 `sha256:59e2100a06da1db8c44994f2b12718aa755fe933e86174a44a6aa63a89c90e0a`) — both under `gsoci.azurecr.io/giantswarm/`, published like 2.2.0; no charts. Consumers: the Substrate line's `images.agentgateway` (giantswarm/substrate#81) |
| **2.2.0** (2026-09-24, tag `v2.2.0` on `71f1adf6` = the 2026-09-24 re-pin onto `main` @ `3528a428` with the six carried patches replayed + the grpc pin at v1.83.2 (GO-2026-6443) + the ledger (#27); a minor of the line for the re-pin; CircleCI `publish` workflow of the tag green, `devctl release wait` exit 0; proven in agentlab on the re-pin's dev build with the Substrate and kagent lines' dev builds: platform-test, a2a-test, skills-test public and private, agents-test) | `main` @ `3528a428` | `agentgateway-upstream/agentgateway:2.2.0` `sha256:370827be39bb2d8040af02cad69e0163e6ed0382c5565839dff4eadcbeea4b56` (linux/amd64 `sha256:0df237aca4d3cb74c9bad490585ee5ba6cd938b5b897f17ab5b7cbdb13a5c21a`, linux/arm64 `sha256:389d50723633ab8489cc9d84c77209e6429f560cda6bcb55e384335e8e596172`) · `agentgateway-upstream/controller:2.2.0` `sha256:5502d4f18c63a0b9cde577440ae7248532287d3b408ddeeca6f069d67f2b8c0d` (linux/amd64 `sha256:f7bfaacb8beea5b18ebb2009d8e4994cc9347c4be59cf5e8a37d29ed0893d4d3`, linux/arm64 `sha256:caa65ca59cf3869491786fe9f20e23e22d3c8e6e6ce981a765c3edec4bd84639`) — both under `gsoci.azurecr.io/giantswarm/`, published like 2.1.2; no charts. Consumers: the Substrate line's egress and router gateways from its `1.1.0` (giantswarm/substrate#74); the packaging chart and the platform's controller and data planes follow with their own bumps |
| **2.1.2** (2026-09-23, tag `v2.1.2` on `39023841` = 2.1.1 + the files a config parse reads made managed dependencies and the watch refresh re-armed (#23, rebase) + the e2e warm step's absolute `-toolexec` path (#21) + the ledger (#19); CircleCI `publish` workflow of the tag green, `devctl release wait` exit 0) | `main` @ `08f4563b` | `agentgateway-upstream/agentgateway:2.1.2` `sha256:a29ec8796363013f633bc04357392248619907c7a14bd5c481ee56f0b8acbd6a` (linux/amd64 `sha256:36d31954f5125dc1ce526e842c6659d0fedc1812249d046774959bcc27688cb2`, linux/arm64 `sha256:884899fbed473f0817e2dd9d3f1439e0a7470d837e4dbef34ae1d0ef779b7d21`) · `agentgateway-upstream/controller:2.1.2` `sha256:30116ff995b4e66f710173772fe74e56fb97c3358cf9e398c5c0a99cded98191` (linux/amd64 `sha256:1a5be548cf4e3433776590ffea9ee71e75e529ee6a2005a13e30b8453b8e5569`, linux/arm64 `sha256:74a6ec222e477f4f6e077590b7f0d876e9e62b01aa92dce221070a1872853894`) — both under `gsoci.azurecr.io/giantswarm/`, published like 2.1.1; no charts. Consumers: the Substrate line's egress and router gateways (release-1.0 v1.0.3, giantswarm/giantswarm#37915), the packaging chart (giantswarm/agentgateway#66), the platform's controller and data planes with agent-platform#648 |
| **2.1.1** (2026-09-23, tag `v2.1.1` on `db201f5e` = 2.1.0 + the file resources read from disk on every managed fetch and re-read every 60 s (#20, squash) + the align-files update (#18); CircleCI `publish` workflow of the tag, every job green: both native data-plane builds, both pushes, both China syncs, both Trivy scans) | `main` @ `08f4563b` | `agentgateway-upstream/agentgateway:2.1.1` `sha256:825c2f9e7156c59f333c840fc06c80f8cf3056a6f08e7646d58c3727871689e4` (linux/amd64 `sha256:69140cb9a724d912dea8de7341e3a5ece556868d6b11204b5bedc054a9edc239`, linux/arm64 `sha256:5415e673767ffdbb1bc21a4698f3e8e6625ed3d5b30b272ea4e68d2454049560`) · `agentgateway-upstream/controller:2.1.1` `sha256:adebf8c99ef67bedd687ab51ddfc25f54000d832f9ed7237bf222fb787cb360c` (linux/amd64 `sha256:7078d64656e605d48c7842c5ac542701191f3ef59ea8640b9dd88f99d879fffe`, linux/arm64 `sha256:70e7e53c1a54303430b9474c09620bd8d91442796ea7ef5a84d9d888cea06129`) — both under `gsoci.azurecr.io/giantswarm/`, cosign-signed with the org's CircleCI identity, SPDX SBOM and provenance attached, index annotation `io.giantswarm.upstream.version=main@08f4563b`, Trivy clean, mirrored to the China registry; no charts (see "Publishing"). Consumers: the Substrate line's egress and router gateways first (giantswarm/giantswarm#37915: the rotated podidentity certificate), through the Substrate release's `images.agentgateway` and the meta chart's `substrate.images.agentgateway`; the packaging chart (v2.6.0) and the platform's controller and data planes (agent-platform 4.65.0) followed the same evening |
