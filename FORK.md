...
| When it moves | with the consumers, proven in agentlab first — see "Re-pin". Not on a schedule. Previous pins: v1.5.0 (2026-09-10 → 2026-09-14, releases `v1.5.1-gs.1`–`gs.3`), `main` @ `c1d24607` (2026-09-14 → 2026-09-22, releases `v1.5.1-gs.4`, `v1.5.1-gs.5`, `2.0.0`). |
...
Three patches change agentgateway's behaviour beyond upstream `main` at the pin: the admission of a resuming actor,
the `GRPCRoute` method-match translation and the named buffer error on an MCP upstream's response, all written for
upstream and leaving at the first upstream commit that carries them. Dropped at the 2026-09-22 re-pin because the pin contains them: the `cherry-pick -x` of upstream
